...
- `L/W/I` の指サイン、`T/Y/X/O` の定型ポーズ、`Heart Arms` を追加
- ノード下部の表示欄は、JSONではなく出力予定の自然文プロンプトを表示
- ドロップダウン変更だけで、下部の `Prompt Preview` が即時更新されます
...
## 表示欄
ノード下部には、出力予定の自然文プロンプトが表示されます。ドロップダウンを変更すると実行前でも即時更新され、実行後の `pose_text` と同じ内容を確認できます。旧 `custom_add_on` 欄は内部互換用として残し、通常のUIには表示しません。
